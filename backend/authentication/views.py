...
class SignupView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if (
            email.endswith("@lmu.edu.ng")
            or email.endswith("@brightdata.com")
            or email.endswith("@dev.to")
        ):
            add_credits(user, amount=5, reason="Signup bonus for LMU or Bright Data")
        ...
        message = render_to_string(
            "authentication/email_confirmation_email.txt",
            {
                "user": user,
...
class LoginView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.is_2fa_enabled:
            response = Response(
                {"detail": "2FA required.", "user_id": user.id},
...
class TwoFactorVerifyView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"detail": "Invalid user ID."}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if not totp.verify(code):
            return Response(
                {"detail": "Invalid 2FA code."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ResetPasswordView(generics.UpdateAPIView):
    ...
    def post(self, request, uidb64, token, *args, **kwargs):
        serializer = self.serializer_class(
            data=request.data,
            context={"request": request, "kwargs": {"uidb64": uidb64, "token": token}},
        )
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(
...
class TwoFactorToggleView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            {
                "detail": f"Two-factor authentication {status_message}.",
...
# views.py (update existing TokenRefreshView)
class TokenRefreshView(APIView):
    permission_classes = [AllowAny]

    def post(self, request):
        # Correctly retrieve the refresh token cookie using settings
