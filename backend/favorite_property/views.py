...
class FavoritePropertyListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return FavoriteProperty.objects.filter(user=self.request.user).order_by('-added_at')

    def perform_destroy(self, instance):
        Schedule.objects.filter(name=f'price_check_user_{instance.user.id}_{instance.property.id}').delete()
        instance.delete()
...
class AddFavoriteProperty(APIView):
    ...
    def post(self, request):
        ...
        if not url:
            return Response({"error": "Missing `details_url` query parameter."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            if not listing_data_list:
                return Response({'success': False, 'error': 'No listings found for the provided URL.'}, status=status.HTTP_404_NOT_FOUND)
            
            listing_data = listing_data_list[0]  

            try:
                parsed_listing_date = datetime.strptime(listing_data.listing, "%d %b %Y") if listing_data.listing else None
            except ValueError:
            ...
            return Response({'success': False, 'error': f"Failed to process URL ({url}): {str(e)}"}, status=status.HTTP_400_BAD_REQUEST)
...
class FavoritePropertyByURLView(APIView):
    ...
    def get(self, request):
        ...
        try:
            favorite = FavoriteProperty.objects.get(user=request.user, property__details_url=url)
    ...
    def delete(self, request):
        ...
        try:
            ...
            Schedule.objects.filter(name=f'price_check_user_{favorite.user.id}_{favorite.property.id}').delete()
            favorite.delete()
            return Response({'success': True, 'message': 'Favorite deleted'}, status=status.HTTP_204_NO_CONTENT)
        except FavoriteProperty.DoesNotExist:
            return Response({'error': 'Favorite not found'}, status=status.HTTP_404_NOT_FOUND)
